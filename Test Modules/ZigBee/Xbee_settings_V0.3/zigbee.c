...
void ZIGB_wr_page();
void ZIGB_menu_page();
static void ZIGB_node_init(U16);

ZB_NODE  zb_node[10];
ZB_NODE  *free_zb_list;
...
void ZIGB_my_read()
{
  ...
  for(temp_cnt = 0; zb_dev.my[temp_cnt - 1] != '\r'; temp_cnt++)
  {
   	while((U1LSR & 0x01) == 0);
   	zb_dev.my[temp_cnt] = U1RBR;
  }

  zb_dev.my[temp_cnt - 1] = '\0';

  uart0_send_string("\n\n\rMY : ");
  uart0_send_string(&zb_dev.my[0]);
}
...
void ZIGB_pan_read()
{
  ...
  for(temp_cnt = 0; zb_dev.pan[temp_cnt - 1] != '\r'; temp_cnt++)
  {
   	while((U1LSR & 0x01) == 0);
   	zb_dev.pan[temp_cnt] = U1RBR;
  }

  zb_dev.pan[temp_cnt - 1] = '\0';

  uart0_send_string("\n\n\rPAN: ");
  uart0_send_string(&zb_dev.pan[0]);
}
...
void ZIGB_ch_read()
{
  ...
  for(temp_cnt = 0; zb_dev.ch[temp_cnt - 1] != '\r'; temp_cnt++)
  {
   	while((U1LSR & 0x01) == 0);
   	zb_dev.ch[temp_cnt] = U1RBR;
  }

  zb_dev.ch[temp_cnt - 1] = '\0';

  uart0_send_string("\n\n\rCH : ");
  uart0_send_string(&zb_dev.ch[0]);
}
...
void ZIGB_dl_read()
{
  ...
  for(temp_cnt = 0; zb_dev.dl[temp_cnt - 1] != '\r'; temp_cnt++)
  {
   	while((U1LSR & 0x01) == 0);
   	zb_dev.dl[temp_cnt] = U1RBR;
  }

  zb_dev.dl[temp_cnt - 1] = '\0';

  uart0_send_string("\n\n\rDL : ");
  uart0_send_string(&zb_dev.dl[0]);
}
...
void ZIGB_node_disc()
{
  ...
  uc_no_cr = LIB_frag_arr(Uc_recv_data, temp_arr,'\r');
 
  uc_no_cr = (uc_no_cr / 6 ); 

  ZIGB_node_init(uc_no_cr);
  pzb = free_zb_list;
  free_zb_list = pzb->next;
 
  uart0_send_string("\n\n\rDevices searched: ");
  uart0_send_byte(uc_no_cr + 0x30);

  uart0_send_string("\n\n\r");
  uart0_send_string(Uc_recv_data);

  for(uc_zb_id = 0; uc_zb_id < uc_no_cr; uc_zb_id++)
  {
   str_copy(temp_arr[(uc_zb_id * 6) + 0], pzb->my, '\r');
   str_copy(temp_arr[(uc_zb_id * 6) + 1], pzb->serial_high, '\r');
   str_copy(temp_arr[(uc_zb_id * 6) + 2], pzb->serial_low, '\r');
   str_copy(temp_arr[(uc_zb_id * 6) + 3], pzb->db, '\r');
   
   uart0_send_string("\n\n\rRemote MY: ");
   uart0_send_string(pzb->my);
      
   uart0_send_string("\n\n\rRemote SH: ");
   uart0_send_string(pzb->serial_high);
      
   uart0_send_string("\n\n\rRemote SL: ");
   uart0_send_string(pzb->serial_low);
      
   uart0_send_string("\n\n\rDB: ");
   uart0_send_string(pzb->db);
...
/**********************************************************************************/
/* Function Name: ZIGB_node_init()												  */
/* Arguments	: NONE															  */
/* Return Types	: NONE															  */
/* Description	: Initializes the structures for Zigbee nodes					  */
/*				  																  */
/**********************************************************************************/

static void ZIGB_node_init(U16 no_zb)
{
 U8 uc_cnt = 0;
 ZB_NODE *pzb1;
 ZB_NODE *pzb2;
 ...
 for(uc_cnt = 0; uc_cnt < no_zb; uc_cnt++)
 {
  pzb1->next = pzb2;
  pzb2->prev = pzb1;
  pzb1++;
