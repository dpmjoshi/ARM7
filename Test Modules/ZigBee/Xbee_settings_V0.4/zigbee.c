...
U8 ZIGB_responce(U8 resp_cnt, U8 *resp)
{
  ...
  if(LIB_str_cmp(&temp_arr[0],resp))
   {
	 return 1;
   }
  else
...
void ZIGB_read()
{
 ...
 lcd_line1_disp(&Uc_zigb_my[0],10);
 lcd_line2_disp(&Uc_zigb_pan[0],10);
 lcd_line3_disp(&Uc_zigb_ch[0],10);
