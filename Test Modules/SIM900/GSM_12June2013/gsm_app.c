...
CONTACT contactlist[50];
CONTACT *contactlist_head;
CONTACT *contactlist_tail;
...
/**********************************************************************************/
/* Function Name: GSM_APP_contactlist_init									  	  */
/* Arguments	: 			 												  	  */
/* Return Types	: 																  */
/* Description	: 															      */
/**********************************************************************************/
void GSM_APP_contactlist_init(U8 contactlist_size)
{
  	CONTACT *contact1;
	CONTACT *contact2;
	...
	contact1->prev_no = 0;
 	...
 	while(contactlist_size)				  // Preparing a doubly link list 
 	{									  
	 	contact2->prev_no = contact1;		  
		contact1->next_no = contact2;	
		contact1->cnt = FREE;
		contact1++;
		contact2++;
		contactlist_size--; 
	...
	contactlist_head    = &contactlist[0];		   // Assigning Head of the list
	contactlist_tail    = contactlist_head;		   // At initialization Head & Tail are same.
	// unlike diallist this list is not circular 
}
/**********************************************************************************/
/* Function Name: GSM_APP_contactlist_update								  	  */
/* Arguments	: 			 												  	  */
/* Return Types	: 																  */
/* Description	: 															      */
/**********************************************************************************/
void GSM_APP_contactlist_update(U8 *no, U8 *name)
{
	// sorting algorithm will first sort the new no & then will insert it at 
	// appropriate position
	U8 uc_result = 4;
	CONTACT *contact_temp;

	contact_temp = contactlist_tail;
	LIB_str_copy(no, &contactlist_head->no[0], 10);	 // Common part for both cases 
	LIB_str_copy(name, &contactlist_head->name[0], LIB_strlen(name));		 // In any of the case New No is added at the Head of the list
	contactlist_head->cnt = NOT_FREE;
		
	//contactlist_head = new_contact;

	if(contactlist_tail == NOT_FREE)
	{
		while((uc_result != 2) || (contact_temp->cnt == NOT_FREE))		// comparing two nos unless New no is not greater than first one OR 
		{																// we do not reach to the end of list
			uc_result = GSM_APP_compare_no(contact_temp->no, contactlist_head->no);
			contact_temp = contact_temp->next_no;		
		}
	
		contactlist_head->prev_no = contact_temp->prev_no ;				// disconnect the exsiting link
		contact_temp->prev_no = contactlist_head;
		contact_temp = contactlist_head->prev_no;
		contact_temp->next_no = contactlist_head;
	}
	else
	{
	 	contactlist_tail = contactlist_head; // this contdion will be true for the first time only when list is empty
		contactlist_head = contactlist_tail->next_no;


	}

	if(contactlist_head->next_no == NOT_FREE)	// Check whether new node is available or not 
	{
	 	contactlist_head = contactlist_head->next_no; // finding New HEAD for the contactlist
	}
...
/**********************************************************************************/
/* Function Name: GSM_APP_coompare_no										 	  */
/* Arguments	: 			 												  	  */
/* Return Types	: 																  */
/* Description	: 															      */
/**********************************************************************************/

U8 GSM_APP_compare_no(U8 *no1, U8 *no2)
{
  	while(*no1 == *no2)
	{
		no1++;
...
{
	U8  *uc_ptr, *temp_msg;		  // uc_ptr is used to navigate whithin a individual arrays of dictionary,  
	U8  cur_key = 0;			  // temp_msg pointer is used as pointer to msg array
	U8  prev_key = 0;
	U8  *msg_arr[15] = {" 0", ".,;:1", "abc2", "def3", "ghi4", "jkl5", "mno6", "pqrs7", "tuv8", "wxyz9"}; 
	U8  caps = 0;				  // all the arrays above are individual arrays

	temp_msg = text_msg;		  // taking copy of pointer.
	clrscr();
	...
	{	
		...
		{
			if(cur_key == prev_key)			// this signifies that entered key is equal to the previous key
			{
	  			if(*(uc_ptr + 1) == 0)
	  			{
					uc_ptr = msg_arr[cur_key - 0x30];  // set the starting address if user has been reached to end of array
				}
	  			else
	  			{									   // increasing uc_ptr allows user to navigate through all elements of an individual array
	   				uc_ptr++;
	   			}
				*temp_msg = *uc_ptr;  		 // writing a letter into msg_array
				if(caps == 1)
				{
				 	*temp_msg -= 0x20;		// converting letters into CAPS
				}
			}
			else 						   // will enter in this loop when prev_key is not equal to the cur_key
			{
